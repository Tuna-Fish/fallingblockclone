...
pub fn render_board(
    ...
    if let Ok((mut vis, mut sprite, mut trans)) = board_bg.single_mut() {
        *vis = if is_visible {
            Visibility::Visible
        } else {
            Visibility::Hidden
    ...
    if let Ok((mut vis, mut sprite, mut trans)) = next_bg.single_mut() {
        *vis = if is_visible {
            Visibility::Visible
        } else {
            Visibility::Hidden
    ...
    // Draw current piece
    if let Ok(piece) = current_piece.single() {
...
pub fn update_ui(
    ...
    mut hud_container: Query<&mut Node, With<HudContainer>>,
    ...
    if let Ok(mut style) = hud_container.single_mut() {
    ...
    if let Ok(mut text) = score_text.single_mut() {
        match *app_mode {
            ...
            AppMode::HighScore => {
                text.0 = format!("Last Score: {:>5}", game_state.score);
            }
        }
    }

    if let Ok((mut text, mut visibility)) = high_score_text.single_mut() {
